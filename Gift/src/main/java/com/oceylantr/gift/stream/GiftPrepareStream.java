...
public class GiftPrepareStream {
	
	public static void main(final String[] args) throws Exception {
		
		Properties props = new Properties();
		props.put(StreamsConfig.APPLICATION_ID_CONFIG, "gift_prepare");
		...
		filtered.map((key, event) -> {
			return new KeyValue<String, Event>(
					key, new Event(
							...
							EventTypes.gift_prepared, 
							new Date(), 
							event.getEventDetail())); //name_surname var burada
		})
		.to("event_table");
