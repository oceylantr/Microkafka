...
public class HardwarePlaceStream {
	
	public static void main(final String[] args) throws Exception {
		
		Properties props = new Properties();
		props.put(StreamsConfig.APPLICATION_ID_CONFIG, "hardware_place");
...
//		props.put(ConsumerConfig.SESSION_TIMEOUT_MS_CONFIG, 6000);
		...
		KStream<String, Event> filtered = source.filter(
				(key, event) -> isHardwareOrderedEvent(event));
	...
	private static boolean isHardwareOrderedEvent(Event event) {
		return EventTypes.hardware_ordered.equals(event.getEventType());
	}
