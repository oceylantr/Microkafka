...
public class PersonLocateStream {
	
	public static void main(final String[] args) throws Exception {
		
		Properties props = new Properties();
		props.put(StreamsConfig.APPLICATION_ID_CONFIG, "person_locate");
...
//		props.put(ConsumerConfig.SESSION_TIMEOUT_MS_CONFIG, 6000);
		...
		KStream<String, Event> filtered = source.filter(
				(key, event) -> isGiftPreparedEvent(event));
		...
		filtered.map((key, event) -> {
			return new KeyValue<String, Event>(
					key, new Event(
							...
							EventTypes.person_located, 
							new Date(), 
							"person#" + event.getEventDetail() + "#location" + "#2.Kat 51 no'lu masa"));
		})
		.to("event_table");
	...
	private static boolean isGiftPreparedEvent(Event event) {
